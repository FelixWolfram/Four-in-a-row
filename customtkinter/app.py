...
def set_chip_version_1(self, col):
    ...
    if self.next_free_row[col] < 0:
        ...
        self.player1 = not self.player1  # change player
        self.print_player()
        if (winner := check_for_winner(self.board_height, self.board_width, self.board,
                                       self.next_free_row)) != 0:  # one player won
            self.game_end(winner)
            return
        self.computer_move()
...
def computer_move_version_1(self):
    if self.player1:
        chip_col = self.colors["chip_col1"]  # save color for the "chip"
    else:
        chip_col = self.colors["chip_col2"]
...
def animate_chip_version_1(self, x_coord_chip, y_coord_chip, y_coord, move_y):
    y_coord_chip += move_y
